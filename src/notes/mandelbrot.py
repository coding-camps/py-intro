...
def plot_mandelbrot(): #定义绘制mandelbrot图像
    X=np.linspace(-1.75,1.05,1000) #实部范围，5000这个数要量力而行
    Y=np.linspace(-1.25,1.25,1000) #虚部范围，5000这个数要量力而行
    real,image=np.meshgrid(X, Y) #生成网格点坐标矩阵。
    c=real+image*1j #构造复数
    mandelbrot_set = np.frompyfunc(iterator, 3, 1)(c, 1.5, 100).astype(float) #frompyfunc(func, nin, nout)，其中func是需要转换的函数，nin是函数的输入参数的个数，nout是此函数的返回值的个数,frompyfunc把Python里的函数（可以是自写的）转化成ufunc
